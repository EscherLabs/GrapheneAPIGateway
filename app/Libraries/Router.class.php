...
class Router
{
  ...
  static private function fetch_args($path,$help_params) {
    ...
    if (count($missing_args)>0) {
      return response(json_encode(['error'=>'You must provide: '.implode(',',$missing_args)]),400)
      ...
      if (preg_match($path_regex, $thisRoute) == 1) {
        foreach($path_info as $verb => $path_info_info) {
          if($_SERVER['REQUEST_METHOD'] == $verb || $verb == 'ALL') {
            ...
            ob_start();
            $response = self::fetch_args($path,$path_info_info['argsinfo']);
            if (!is_array($response)) { /* Got an Error */
              return $response;
            } else {
              $args = $response;
            }
